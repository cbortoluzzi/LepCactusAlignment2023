...
def get_coverage_of_each_replicate(files, base_pair):
	"""
	Calculate coverage as proportion of sites in species A that aligns to n or more sites in species B
	Input:
		files : list of all coverage files generated by maf_stream coverage
		base_pair : length of random interval (by default this is 1Mb)
	Output:
		mydict : dictionary where the key is the name of the target species (as in the cactus alignment) and the value is the coverage expressed as %
	"""
...
def get_species_group(species, mydict):
	...
	Output:
		myspecies : dictionary reporting the species name (as in the cactus alignment), group, and coverage expressed as %
	"""
	myspecies = {}
