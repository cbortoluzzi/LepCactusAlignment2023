...
do
	...
	do
		...
		do
			# Get the genomic coordinates of each complete, single-copy BUSCO gene as they appear in the protein FASTA file generated by BUSCO
			coordinates=`cat $faa | grep '>' | sed 's/:/\t/g' | sed 's/>//g' | sed 's/-/\t/g'`
			echo $coordinates $species $genome >> busco/$busco_id.tsv
		done
	done
	# We now need to change the contig/chromosome name because the one present in the protein FASTA file is based on the NCBI annotation, whereas the contig/chromosome name in the alignment is a digit-based value
	python3 change_contig_name.py --species $FILE --busco busco/$busco_id.tsv && rm busco/$busco_id.tsv

	# We can now use the first species in the newly annotated file as a reference to extract from the cactus alignment a sub-alignment in MAF format of all complete, single-copy BUSCO genes
	...
	# Finally, check the integrity of the alignment: we will take 100 bp upstream and downstream to have some flexibility
