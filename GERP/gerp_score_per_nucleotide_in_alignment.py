...
def get_alignment_positions(input):
...
def assign_gerp_score_to_positions(mygerp, myrange, output_file):
	with open(output_file, 'w') as f:
		for chromosome in myrange[0]:
			# Make sure that all positions are sorted !!
			list_positions = sorted(myrange[0][chromosome])
...
if __name__ == "__main__":
	...
	input = list(zip([args.maf], [refGenome]))
	list_positions = pool.map(get_alignment_positions, input)
	pool.close()
	pool.join()
